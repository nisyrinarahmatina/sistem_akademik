...
<div id="layoutSidenav">
    ...
    <main>
        <div class="container-fluid px-4">
            <div class="card mt-3">
            <div class="card-body">
                <h5 class="card-header bg-primary text-white text-center">Daftar Nilai</h5>
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @forelse($nilai as $key => $data)
                        <tr>
                            <td>{{ $loop->iteration }}</td>
                            <td>{{ $data->nama }}</td>
                            <td>{{ $data->kelas }}</td>
                            <td><a href="{{ asset('storage/' . $data['file_nilai']) }}" target="_blank">Download File</a></td>
                            <td>
                                    <!-- Delete Button -->
                            ...
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="deleteModalLabel{{ $data->id }}">Konfirmasi Hapus Nilai</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                            <span aria-hidden="true">&times;</span>
                                        </button>
                                    </div>
                                    <div class="modal-body">
                                        Anda ingin menghapus nilai ini?
                                    </div>
                        ...
                        @empty
                        <tr>
                            <td colspan="8" class="text-center text-muted">Belum ada data nilai. Silakan tambahkan jadwal.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <a href="{{ route('nilai.create') }}" class="btn btn-primary mt-3">Tambah Data</a>
    </main>
    </div>

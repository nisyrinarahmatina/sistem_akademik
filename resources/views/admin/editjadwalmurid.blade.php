...
<div id="layoutSidenav">
    ...
    <main>
        <div class="container">
            ...
            <form action="{{ route('jadwal.updateJadwalMurid', ['id' => $jadwal->id]) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="nama_murid" class="form-label">Nama Murid</label>
                    <input type="text" class="form-control" id="nama_murid" name="nama_murid" value="{{ old('nama_murid', $jadwal->nama_murid) }}" required>
                </div>
                ...
                <a href="{{ route('jadwal.indexJadwalMurid') }}" class="btn btn-secondary mb-3">Batal</a>
            </form>
            @if ($errors->any())
